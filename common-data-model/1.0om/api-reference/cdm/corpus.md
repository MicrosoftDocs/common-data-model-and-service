...
|**FetchObjectAsync\<T>(string, [CdmObject](cdmobject.md), bool, bool, Class<T>)**<br/>*objectPath*: The object path of a folder or document. The path can be absolute or relative.<br/>*obj [optional]*: When provided, it's used to obtain the *FolderPath* and *Namespace* that are needed to create the absolute path from a relative path.<br/>*shallowValidation [optional]*: When provided, shallow validation in [ResolveOptions](../utilities/resolveoptions.md) is enabled, which logs errors regarding loading and resolving object references as warnings instead.<br/>*forceReload [optional]*: When true, the document containing the requested object is reloaded from storage to access any external changes made to the document since it may have been cached by the corpus.<br/>*classInterface [optional]*: Only in Java, the class of the object that is being fetched. This is optional but recommended to avoid casting exceptions when the object that is fetched does not match with the desired object type.|Fetches the object in the specified path from the corpus.|Task\<T>|
|**FetchObjectAsync\<T>(string, [CdmObject](cdmobject.md), [ResolveOptions](../utilities/resolveoptions.md), bool, Class<T>)**<br/>*objectPath*: The object path of a folder or document. The path can be absolute or relative.<br/>*obj [optional]*: When provided, it's used to obtain the *FolderPath* and *Namespace* that are needed to create the absolute path from a relative path.<br/>*resOpt [optional]*: The resolve options.<br/>*forceReload [optional]*: When true, the document containing the requested object is reloaded from storage to access any external changes made to the document since it may have been cached by the corpus.<br/>*classInterface [optional]*: Only in Java, the class of the object that is being fetched. This is optional but recommended to avoid casting exceptions when the object that is fetched does not match with the desired object type.|Fetches the object in the specified path from the corpus.|Task\<T>|
