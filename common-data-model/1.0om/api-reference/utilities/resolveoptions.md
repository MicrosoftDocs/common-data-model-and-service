...
|StrictValidation|bool|When enabled will instruct the Object Model to load all the imports associated with the file being fetched and validate all the references in the documents in [CdmCorpusDefinition.FetchObjectAsync(...)](../cdm/corpus.md#methods). If no value is supplied, the imports and references will be loaded as they are required during the execution of an operation. <br/><br/>This flag is especially useful when dealing with resolved entities since it will reduce the load time of the document.|
